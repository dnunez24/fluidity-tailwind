...
In left-to-right text, `f6y-start-2` sets `inset-inline-start`, which becomes left; in right-to-left, it becomes right.
The physical properties (`left`, `right`, `top`, `bottom`) never change regardless of direction.

**Position requirement:** These utilities set only the edge values; they do not set `position`.
Your element must have `position: relative`, `position: absolute`, `position: fixed`, or `position: sticky` to be positioned at all.
This is a standard Tailwind pattern — add a position class alongside your fluidity utilities, e.g. `absolute f6y-inset-2/8`.
...
Use it alongside scroll snap containers or when JavaScript calls `.scrollIntoView()` to ensure the element doesn't disappear under fixed headers.
...
Swap `f6y-inset-2/8` for `f6y-exp-inset-2/8` to ease the ramp at the start, or `f6y-log-inset-2/8` to ease at the end — the endpoints stay the same, only the path between them changes.
