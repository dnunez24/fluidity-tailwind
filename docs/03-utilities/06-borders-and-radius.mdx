...
Widths accept px counts or arbitrary values; offset accepts spacing-scale numbers or arbitrary values.

:::note
The `lineWidth` kind is unique: bare numbers represent pixel counts (e.g. `border-2` → `2px`), not spacing-scale multiples.
