...
:::note
Unlike Tailwind's own `space-x-*` and `space-y-*`, fluidity's space-between utilities do **not** have a `-reverse` variant. For reversed direction, apply the utility to a flex or grid container with `flex-row-reverse` or `grid-auto-flow: column-reverse` instead.
:::
