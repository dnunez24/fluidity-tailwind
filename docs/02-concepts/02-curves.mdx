...
Every fluidity utility is available in three curve variants: linear, exponential, and logarithmic.
Each uses a different ease function that transforms progress into a value.
All three share identical endpoints (min at progress 0, max at progress 1), but diverge between them.
...
Swapping `f6y-p-2/8` for `f6y-exp-p-2/8` changes nothing at the viewport boundaries.
