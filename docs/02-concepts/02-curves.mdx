...
Every fluidity utility is available in three curve variants—linear, exponential, and logarithmic—each with a different ease function that transforms progress into a value.
All three share identical endpoints (min at progress 0, max at progress 1), but diverge in between.
...
Higher bases (e.g., 4, 8) make the curve sharper—more of the range stays compressed near the lower bound.
Lower bases (e.g., 1.5) flatten the curve.
...
The value climbs quickly near the lower bound, then levels off as it approaches the upper bound.
...
All three use identical min (0.5rem) and max (2rem) bounds, but the padding value follows a different path between them.
...
Swapping `f6y-p-2/8` for `f6y-exp-p-2/8` changes nothing at the viewport boundaries; only the path in between changes.
