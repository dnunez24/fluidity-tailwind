...
The `ease` function then transforms that progress (linear, exponential, or logarithmic), and the result scales a value from `min` to `max`.

## Why `tan(atan2())`?

CSS `calc()` cannot divide a length by a length.
