...
Higher bases (4, 8) create sharper curves; lower bases (1.5) flatten them.

In the demo below, the left box uses the default base 2, and the right box forces base 4.
Both use `f6y-exp-p-2/8`.
Notice how base 4 keeps padding much closer to the min (0.5rem) until the very end, then ramps faster.
...
When you omit the max value (e.g., `f6y-p-2` instead of `f6y-p-2/8`), the max is computed as `min × --f6y-ratio`.
