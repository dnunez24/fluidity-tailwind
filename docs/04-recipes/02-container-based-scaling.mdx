...
A reusable card component needs to look right in every layout — 1-column mobile, 2-column tablet, 3-column desktop grid.
Rather than writing per-breakpoint overrides for each grid context, set the card's bounds to scale off its own container width using `f6y-axis-cqi`, so the same card adapts automatically.
...
`f6y-lower-*` and `f6y-upper-*` set the ramp range: padding and text grow smoothly as the card's container grows from 12rem to 48rem.
...
Each grid cell is a `@container` so its child card can measure the column width with `cqi` — no per-breakpoint card overrides needed.

## Why it works

Container query units decouple a component from the viewport.
A card in a sidebar (narrow) and a card in a main grid (wide) can use the exact same fluid utility classes because `cqi` measures each card's own ancestor container, not the viewport.
