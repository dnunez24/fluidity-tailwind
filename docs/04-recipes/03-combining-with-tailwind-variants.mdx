...
description: Layer dark mode, hover states, and breakpoint overrides on top of fluid utilities, which compile to ordinary Tailwind @utility rules.
sidebar:
...
The only special consideration: interactive states (hover, focus, active) that toggle fluidity's values need a CSS transition to animate.
If you omit the transition, the value jumps instantly between states on resize or state change.
