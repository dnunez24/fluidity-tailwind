<!--
  This is the canonical README: it ships in the npm tarball (see `files` in
  package.json) and doubles as the monorepo's root README via a symlink
  (`../README.md -> fluidity-tailwind/README.md`). Edit only this file.

  Why the symlink points this way and not the other way around: npm's and
  pnpm's packers refuse to follow a symlink that points *outside* the
  package directory (`fluidity-tailwind/README.md -> ../README.md` silently
  drops the file from the published tarball, no warning). Keeping the real
  file inside the package and symlinking the repo root into it is the
  robust fix — CI's "Verify npm package contents" step in
  .github/workflows/ci.yml guards against this regressing.
-->
...
Import fluidity after Tailwind in your CSS entry point:
...
`axis` defaults to `100vw`.
`lower`/`upper` default to Tailwind's `sm` (40rem) and `2xl` (96rem) breakpoints.
...
Fluidity provides three easing curves for every utility:
...
Numeric bare values resolve through the spacing scale (`--spacing()`), exactly like Tailwind's built-in `p-*`/`w-*`/etc., except border and outline widths, where bare numbers are px counts (like `border-2` -> `2px`).
Fluidity accepts arbitrary values and theme keys (`--text-*`, `--container-*`, `--leading-*`, `--tracking-*`, `--radius-*`, `--breakpoint-*`) wherever Tailwind's own utilities accept them.

## Configuration

Every configurable value is a custom property with a matching setter utility.
Custom properties inherit, so set one on an ancestor to affect every fluid utility beneath it.
...
| `f6y-ratio-<number>`                      | `--f6y-ratio` | derives max as `min * ratio` when a utility omits max | `2`                             |
| `f6y-axis-<unit>` / `f6y-axis-[<length>]` | `--f6y-axis`  | the measured length                                   | `100vw`                         |

`f6y-axis-*` ships one utility per viewport and container-query unit (`vw`, `vh`, `vmin`, `vmax`, `cqw`, `cqh`, `cqi`, `cqb`, `cqmin`, `cqmax`), plus `f6y-axis-[<length>]` for anything else.
Container units need an ancestor with `container-type` set (Tailwind's `@container`):
...
Every utility in the table ships in linear, `exp-` and `log-` variants (279 `@utility`
...
Targets Baseline Widely Available CSS as of August 2026: `calc()`, `clamp()`, `var()`, the CSS trigonometric functions (`atan2()`, `tan()`) and exponential functions (`pow()`, `log()`), and container query length units (`cqi`, `cqb`, etc.).
No fallback exists for older browsers.
Every declaration a fluidity utility emits embeds `tan(atan2(...))` (plus `pow()`/`log()` for `exp-`/`log-` variants), so an unsupported browser drops the declaration entirely.
The property then falls back to its initial or inherited value (for example, `f6y-p-2/8` produces no padding at all, not a static fallback ramp).

## Out of scope

- **Value validation.**
  Fluidity doesn't check that `min < max`, that units are compatible, or that a curve `base` is greater than 1, the same trust model as Tailwind's own arbitrary values.
- **Third-party plugin integration** (for example, Tailwind Merge) isn't provided.
- **Negative values.**
  Tailwind's `-m-4` idiom has no fluidity counterpart: `-f6y-*` candidates don't match.
  Use arbitrary values for negative ramps (for example, `f6y-m-[-2rem]/[-4rem]`).
...
generated reference of all 279 utilities) live under [`website/docs/`](https://github.com/dnunez24/fluidity-tailwind/tree/main/website/docs), built
